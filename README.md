...
<title>TENDÊNCIA — Você ainda não viu?</title>
<style>
  :root{
...
<header>
  ...
  <div class="brand">
    <h1>TENDÊNCIA</h1>
    <small>Você ainda não viu?</small>
  </div>
...
<div class="container" id="app">
  <aside class="side" id="sideBar">
    <div class="category">
      <h3>Categorias</h3>
      <div class="cat-btn theme-esporte" data-cat="esporte">🏈 Esporte</div>
      <div class="cat-btn theme-moda" data-cat="moda">👗 Moda</div>
      <div class="cat-btn theme-musica" data-cat="musica">🎵 Música</div>
      <div class="cat-btn theme-cinema" data-cat="cinema">🎬 Cinema</div>
      <div class="cat-btn theme-viral" data-cat="viral">🔥 Viral</div>
      <hr />
      ...
      <div>
        <h4>Admin</h4>
        <div id="adminArea" class="muted">Acessar funções restritas</div>
      </div>
    </div>
  </aside>
...
<div id="loginModal" style="display:none;position:fixed;left:50%;top:50%;transform:translate(-50%,-50%);background:var(--card-bg);padding:18px;border-radius:12px;border:1px solid rgba(255,255,255,0.04);width:320px">
  ...
  <div style="display:flex;flex-direction:column;gap:8px">
    ...
    <small class="muted">Contas de teste: admin / admin123  •  membro1 / mem1  •  membro2 / mem2</small>
...
<footer>
  © TENDÊNCIA — Hub de tendências • <span class="pill">Slogan: Você ainda não viu?</span>
</footer>
...
const sampleProducts = [
  { id: 'p1', title:'Tênis Runner X', category:'esporte', price:299.9, margin:0.35, vendor:'forn1', type:'fisico', img:'', trending:120 },
  ...
  { id: 'p4', title:'Coleção Filme Cult', category:'cinema', price:49.9, margin:0.45, vendor:'forn3', type:'fisico', img:'', trending:40 },
...
function renderHome(){
  const html = `
    <div class="hero theme-esporte">
      <div class="cover">
        <h2>As tendências do dia</h2>
        <p class="muted">Um resumo rápido das maiores tendências nas categorias que você ama.</p>
        <div style="display:flex;gap:8px;margin-top:12px">
          <button class="cta" onclick="goExplorar()">Explorar tendências</button>
      ...
      <div class="summary">
        ...
        <div class="card">
          ...
          <div class="muted">R$ 19,90 • +200 views</div>
        </div>
      </div>
    </div>
  ...
  grid.innerHTML = prods.map(p=>`
    <div class="card">
      <div style="height:120px;background:linear-gradient(45deg, rgba(255,255,255,0.02), rgba(0,0,0,0.2));display:flex;align-items:center;justify-content:center;border-radius:8px">
        <strong style="font-size:20px">${p.type==='digital'?'📦':'🛍️'}</strong>
      </div>
      <h3>${p.title}</h3>
      <div class="muted">${p.category} • ${p.vendor}</div>
...
function openProduct(id){
  ...
  view.innerHTML = `
    <div class="hero theme-${p.category}">
      <div class="cover">
        <h2>${p.title}</h2>
        <p class="muted">${p.category} • ${p.vendor}</p>
        <p style="margin-top:12px"><strong>${currency(p.price)}</strong> <span class="muted">• margem ${(p.margin*100|0)}%</span></p>
      ...
      <div class="summary">
        <h4>Sobre</h4>
        <p class="muted">Descrição breve do produto / serviço. Lorem ipsum para preencher.</p>
      </div>
    </div>
...
function goAfiliados(){
  view.innerHTML = `
    <h2>Afiliados & Divulgação</h2>
    <div class="card">
      <p class="muted">Espaço para afiliados cadastrarem links, campanhas e metas. Sistema de comissão por produto (configurável).</p>
      <button onclick="renderAffiliateForm()">Quero ser afiliado</button>
    </div>
    <h3>Campanhas ativas</h3>
    <div id="campanhas" class="grid"></div>
  `;
  document.getElementById('campanhas').innerHTML = state.products.filter(p=>p.type==='fisico').map(p=>`<div class="card"><h4>${p.title}</h4><div class="muted">Comissão sugerida: ${(p.margin*50|0)}%</div></div>`).join('');
}

function renderAffiliateForm(){
  view.innerHTML = `<h3>Cadastro de Afiliado</h3>
  <div class="card"><input placeholder="Nome/Usuário" id="affName"/><input placeholder="Link ou PayPal / Pix" id="affLink" style="margin-top:8px"/><div style="margin-top:10px"><button onclick="submitAff()">Enviar</button></div></div>`;
}
function submitAff(){ alert('Pedido de afiliado enviado! Em produção, criaremos fluxo de aprovação.'); }
...
function importCSV(){
  ...
  reader.onload = e=>{
    ...
    save();
    alert('Importado! Atualize a página se necessário.');
  }
  reader.readAsText(f);
}
...
function renderCartModal(){
  ...
  el.innerHTML = state.cart.map(i=>`<div style="display:flex;justify-content:space-between;gap:8px;padding:6px 0;border-bottom:1px solid rgba(255,255,255,0.02)"><div><strong>${i.title}</strong><div class="muted">${i.type} • ${currency(i.price)}</div></div><div><button onclick="removeCart('${i.cartId}')">Remover</button></div></div>`).join('');
...
document.getElementById('btnCheckout').addEventListener('click', ()=> {
  ...
  <div style="display:flex;gap:8px;margin-top:8px">
    ...
    <button onclick="pay('card')">Cartão (via gateway)</button>
  </div>
  </div>`;
  document.getElementById('orderList').innerHTML = state.cart.map(i=>`<div class="muted">${i.title} — ${currency(i.price)}</div>`).join('');
...
function pay(method){
  ...
  if(method==='mercadopago'){
    alert('Redirecionando para MercadoPago (exemplo). Em produção, você chamará seu backend que cria a preferência e retorna o URL de checkout.');
    // Example: fetch('/api/create_preference', {method:'POST', body:JSON.stringify({cart:state.cart})}).then(...)
  } else if(method==='picpay'){
    alert('PicPay: precisa de integração com API do PicPay no backend. Aqui simulamos.');
  } else if(method==='pix'){
    alert('PIX: em produção, backend gera QR Code e retorna para o cliente; aqui mostramos simulação.');
  } else {
    alert('Cartão: use um gateway (MercadoPago, Stripe, PagSeguro). É necessário backend para tokens seguros.');
  }
}
...
document.getElementById('btnDoLogin').addEventListener('click', ()=>{
  ...
  else { alert('credenciais inválidas (use as contas de teste)'); return; }
  document.getElementById('loginModal').style.display='none';
});

function initAfterLogin(){
  document.getElementById('adminArea').innerHTML = state.user.role==='admin'? '<span class="admin-badge">ADMIN</span> • Editor completo': '<span class="pill">MEMBRO</span>';
...
function openAdminPanel(){
  ...
  <div class="card" style="margin-top:12px"><h3>Configurações</h3>
    <div class="muted">Funções restritas: alterar logo, editar comissões, remover membro, gerar relatórios.</div>
...
function changeLogo(){
  ...
  if(url){ document.getElementById('logoImg').src = url; alert('Logo atualizada visualmente. Para persistir, atualize o arquivo no repositório.'); }
}
